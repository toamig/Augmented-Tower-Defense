...
public class WeatherNetwork
{
    ...
    public delegate void WeatherData(WeatherTypes type);
        ...
        else
        {
            if (info.cod == 200 && info.weather?.Length >= 1)
            {
                foreach (ExternalWeather weather in info.weather)
                {
                    if (weather.id == 800)
                    {
                        OnWeatherDataReceived(WeatherTypes.Clear);
                    }
                    else if (weather.id == 801 || weather.id == 802)
                    {
                        OnWeatherDataReceived(WeatherTypes.FewClouds);
                    }
                    else if (weather.id == 803 || weather.id == 804)
                    {
                        OnWeatherDataReceived(WeatherTypes.Clouds);
                    }
                    else if (weather.id >= 200 && weather.id < 300)
                    {
                        OnWeatherDataReceived(WeatherTypes.HeavyRain);
                    }
                    else if (weather.id >= 300 && weather.id < 400)
                    {
                        OnWeatherDataReceived(WeatherTypes.Drizzle);
                    }
                    else if (weather.id == 500 || weather.id == 511 || weather.id == 520 || weather.id == 531)
                    {
                        OnWeatherDataReceived(WeatherTypes.LightRain);
                    }
                    else if (weather.id == 501 || weather.id == 521)
                    {
                        OnWeatherDataReceived(WeatherTypes.Rain);
                    }
                    ...
                    {
                        OnWeatherDataReceived(WeatherTypes.Snow);
                    }
